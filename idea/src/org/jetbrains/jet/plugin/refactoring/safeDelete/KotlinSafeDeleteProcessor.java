...
public class KotlinSafeDeleteProcessor extends JavaSafeDeleteProcessor {
    ...
    public static boolean isInside(@NotNull PsiElement place, @NotNull PsiElement ancestor) {
        if (ancestor instanceof JetPsiMethodWrapper) {
            ancestor = ((JetPsiMethodWrapper) ancestor).getOrigin();
        }
        ...
        for (PsiMethod overridingMethod : overridingMethods) {
            Collection<PsiReference> overridingReferences =
                    ReferencesSearch.search(
                            overridingMethod instanceof JetPsiMethodWrapper ? ((JetPsiMethodWrapper) overridingMethod).getOrigin() : overridingMethod
                    ).findAll();
    ...
    private static boolean isMultipleInterfacesImplementation(
        ...
        for (PsiMethod superMethod : methods) {
            PsiElement relevantElement = superMethod instanceof JetPsiMethodWrapper ? ((JetPsiMethodWrapper) superMethod).getOrigin() : superMethod;
    ...
    private static boolean checkPsiMethodEquality(@NotNull PsiMethod method1, @NotNull PsiMethod method2) {
        if (method1 instanceof JetPsiMethodWrapper && method2 instanceof JetPsiMethodWrapper) {
            return ((JetPsiMethodWrapper) method1).getOrigin().equals(((JetPsiMethodWrapper) method2).getOrigin());
        }
    ...
    public static void cleanUpOverrides(@NotNull PsiMethod method) {
        ...
        overrideLoop:
        for (PsiMethod overridingMethod : overridingMethods) {
            PsiElement overridingElement = overridingMethod instanceof JetPsiMethodWrapper
                                           ? ((JetPsiMethodWrapper) overridingMethod).getOrigin()
                                           : overridingMethod;
    ...
    private static void addParameter(@NotNull PsiMethod method, @NotNull Set<PsiElement> result, int parameterIndex) {
        if (method instanceof JetPsiMethodWrapper) {
            JetDeclaration declaration = ((JetPsiMethodWrapper) method).getOrigin();
