...
public class ConstantExpressionEvaluator private (val trace: BindingTrace) : JetVisitor<CompileTimeConstant<*>, JetType>() {
    ...
    override fun visitSimpleNameExpression(expression: JetSimpleNameExpression, expectedType: JetType?): CompileTimeConstant<*>? {
        ...
        if (callableDescriptor !is PropertyDescriptor) return null

        if (isEnumProperty(callableDescriptor)) {
            return EnumValue(callableDescriptor)
        }
    ...
    return when(value) {
        ...
        is Float -> {
            if (expectedType != null &&
                    !CompileTimeConstantResolver.noExpectedTypeOrError(expectedType) &&
                    TypeUtils.makeNotNullable(expectedType) == KotlinBuiltIns.getInstance().getDoubleType()) {
...
private fun isEnumProperty(descriptor: PropertyDescriptor): Boolean {
    val classifier = descriptor.getType().getConstructor().getDeclarationDescriptor()
